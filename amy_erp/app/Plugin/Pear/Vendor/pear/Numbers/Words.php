...
class Numbers_Words {
    ...
    public static function getLocales($locales = null) {
        ...
        foreach ($sfiles as $fname) {
            ...
            if (is_file($fname) && is_readable($fname) && (count($locales) == 0 || in_array($lname, $locales))
            ) {
                $ret[] = $lname;
            }
        }
...
//            debug($file);
//            die();
            if (stream_resolve_include_path('Words/Locale/fr.php')) {
//                include_once $file;
                include_once 'Words/Locale/fr.php';
            }
