...
class CaissesController extends AppController {
    ...
    public function beforeFilter() {
        parent::beforeFilter();
        $this->Auth->allow('valide_panier','valide_Devis','valide_Livraison','add_client','liste_reglement');
    }
...
//        header('Content-type: application/json');
        ...
        $this->request->data['Commande']['state'] = "Finalisée";
//        debug("php");
//        die(); 
...
//        header('Content-type: application/json');
        ...
        $this->Commande->recursive = -1;
        $count = $this->Commande->find('count');
        $this->request->data['Commande']['ref'] = "DEV-" . date('y') . date('m') . date('d') . "-" . ($count + 1);
...
//                    die();
    ...
    public function valide_Livraison() {
        $this->autoRender = false;
        header('Cache-Control: no-cache, must-revalidate');
        header('Expires: Mon, 26 Jul 1997 05:00:00 GMT');
//        header('Content-type: application/json');
        ...
        $this->Commande->recursive = -1;
        $count = $this->Commande->find('count');
        $this->request->data['Commande']['ref'] = "BL-" . date('y') . date('m') . date('d') . "-" . ($count + 1);
        $this->request->data['Commande']['type'] = 'Commande';
        $this->request->data['Commande']['last_timbre_price'] = $this->getConfiguration()['Configuration']['price_timbre'];
        $this->request->data['Commande']['avoir'] = 0;
        $this->request->data['Commande']['state'] = "Finalisée";
...
//                $clientController->save_client($data['User']);
        ...
        if ($this->request->is('post')) {
            $data = $this->request->data;
            //$reglements = $this->Reglement->find('all');
            $reglements = $this->Reglement->find('all', array(
                'conditions' => array('Reglement.created <= ' => $data['date']['date_fin'],
                    'Reglement.created >= ' => $data['date']['date_debut']
            )));
